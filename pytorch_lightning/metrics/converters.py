...
def _sync_ddp_if_available(result: Union[torch.Tensor],
                           ...
                           reduce_op: Union[torch.distributed.ReduceOp, str] = 'mean',
                           ) -> torch.Tensor:
    """
    ...
    if torch.distributed.is_available() and torch.distributed.is_initialized():
        divide_by_worldsize = False
        if isinstance(reduce_op, str) and reduce_op == 'mean':
            reduce_op = torch.distributed.ReduceOp.SUM
            divide_by_worldsize = True
        ...
        if divide_by_worldsize:
            result = result / torch.distributed.get_world_size(group=group)

    return result


def numpy_metric(group: Any = torch.distributed.group.WORLD,
                 reduce_op: Union[torch.distributed.ReduceOp, str] = 'mean') -> Callable:
    """
